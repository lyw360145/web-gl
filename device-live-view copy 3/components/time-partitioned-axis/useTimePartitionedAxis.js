...
console.log('👊 ~ props', props)
  ...
  const maxZoom = computed(() => {
    const timeRange = props.value.endTime - props.value.startTime // 总时间跨度（毫秒）
  ...
  const minTime = new Date(props.value.startTime)
  const maxTime = new Date(props.value.endTime)

  // **时间比例尺**
  const scale = computed(() => {
    const res = d3.scaleTime().domain([new Date(props.value.startTime), new Date(props.value.endTime)]).range([40, w.value - 40])
    return res
  })
  ...
  const onAxisClick = (event) => {
    ...
    console.log('👊 ~ currentTime.value-click', currentTime.value)
  ...
  // **更新锚点位置**
  const updateAnchorPosition = (init) => {
    if (!anchorRef.value || !props.value.startTime) return
    const x = scale.value(new Date(init ? props.value.startTime : currentTime.value))
  ...
  const updateVideoThumbnails = () => {
    // 保持响应式更新的核心修改点
    videoThumbnails.value = JSON.parse(JSON.stringify(props.value.associationVOList))
      .map(a => ({
        // 创建新对象保持引用更新
        ...a,
